...
#
lock_dir=_sh11276
# Made on 2018-09-02 15:45 CEST by <frede@darthvader>.
...
X.TH HYPER-V 1 "2018-09-02" "perl v5.26.2" "User Contributed Perl Documentation"
X.\" For nroff, turn off justification.  Always turn off hyphenation; it makes
X.\" way too many mistakes in technical documents.
X.if n .ad l
X.nh
...
it under the \s-1MIT\s0 license.
SHAR_EOF
  (set 20 18 09 02 15 45 17 'hyper-v.1'
   eval "${shar_touch}") && \
...
295160de0786fc938f128491374265ca  hyper-v.1
SHAR_EOF

else
test `LC_ALL=C wc -c < 'hyper-v.1'` -ne 5033 && \
  ${echo} "restoration warning:  size of 'hyper-v.1' is not 5033"
  fi
fi
# ============= msvc-shell ==============
...
X.TH MSVC-SHELL 1 "2018-04-01" "perl v5.26.1" "User Contributed Perl Documentation"
X.\" For nroff, turn off justification.  Always turn off hyphenation; it makes
X.\" way too many mistakes in technical documents.
X.if n .ad l
X.nh
...
it under the \s-1MIT\s0 license.
SHAR_EOF
  (set 20 18 04 02 10 10 00 'msvc-shell.1'
   eval "${shar_touch}") && \
...
31850419c09c8b1ce9703d6139c1b520  msvc-shell.1
SHAR_EOF

else
test `LC_ALL=C wc -c < 'msvc-shell.1'` -ne 5821 && \
  ${echo} "restoration warning:  size of 'msvc-shell.1' is not 5821"
  fi
fi
# ============= yank ==============
...
X.\"
X.IX Title "YANK 1"
X.TH YANK 1 "2018-04-01" "perl v5.26.1" "User Contributed Perl Documentation"
X.\" For nroff, turn off justification.  Always turn off hyphenation; it makes
X.\" way too many mistakes in technical documents.
X.if n .ad l
X.nh
...
it under the \s-1MIT\s0 license.
SHAR_EOF
  (set 20 18 04 02 10 10 00 'yank.1'
   eval "${shar_touch}") && \
  chmod 0644 'yank.1'
if test $? -ne 0
...
84684b3a6ae8d984086e63fbd68a03fd  yank.1
SHAR_EOF

else
test `LC_ALL=C wc -c < 'yank.1'` -ne 7097 && \
  ${echo} "restoration warning:  size of 'yank.1' is not 7097"
  fi
fi
# ============= tmux-256color.tinfo ==============
